...
JSON;
    ...
    public function assertEventSent(?string $project = null): void
    {
        $this->broadcastig->assertPushed((string) new EventsChannel($project), function (array $data) use ($project) {
            ...
            $this->assertSame(
                <<<'HTML'
<pre class=sf-dump id=sf-dump-1629301635 data-indent-pad="  "><span class=sf-dump-note>array:2</span> [<samp data-depth=1 class=sf-dump-expanded>
  "<span class=sf-dump-key>a</span>" => <span class=sf-dump-num>1</span>
  "<span class=sf-dump-key>b</span>" => <span class=sf-dump-note>array:1</span> [<samp data-depth=2 class=sf-dump-compact>
    "<span class=sf-dump-key>c</span>" => <span class=sf-dump-num>3</span>
  </samp>]
...
HTML
                ,
                $data['data']['payload']['payloads'][0]['content']['content'],
            );
    ...
    public function testSendBoolean(): void
    {
        $this->dump(true);
        ...
        $this->broadcastig->assertPushed((string) new EventsChannel('default'), function (array $data) {
            ...
            $this->assertSame('custom', $data['data']['payload']['payloads'][0]['type']);
            $this->assertSame(true, $data['data']['payload']['payloads'][0]['content']['content']);
            $this->assertSame('Boolean', $data['data']['payload']['payloads'][0]['content']['label']);

            return true;
        });
    }

    public function testSendNumber(): void
    {
        $this->dump(1);
        ...
        $this->broadcastig->assertPushed((string) new EventsChannel('default'), function (array $data) {
            ...
            $this->assertSame('log', $data['data']['payload']['payloads'][0]['type']);
            $this->assertSame(1, $data['data']['payload']['payloads'][0]['content']['values'][0]);

            return true;
        });
    }

    public function testSendString(): void
    {
        ...
        $this->broadcastig->assertPushed((string) new EventsChannel('default'), function (array $data) {
            ...
            $this->assertSame('log', $data['data']['payload']['payloads'][0]['type']);
            $this->assertSame('Hello, world!', $data['data']['payload']['payloads'][0]['content']['values'][0]);

            return true;
        });
    }

    public function testSendArray(): void
    {
        $this->dump(['foo' => 'bar']);
        ...
        $this->broadcastig->assertPushed((string) new EventsChannel('default'), function (array $data) {
            ...
            $dumpId = DumpIdParser::find($data['data']['payload']['payloads'][0]['content']['values'][0]);

            $this->assertSame('log', $data['data']['payload']['payloads'][0]['type']);
            $this->assertSame(
                <<<HTML
<pre class=sf-dump id={$dumpId} data-indent-pad="  "><span class=sf-dump-note>array:1</span> [<samp data-depth=1 class=sf-dump-expanded>
  "<span class=sf-dump-key>foo</span>" => "<span class=sf-dump-str title="3 characters">bar</span>"
</samp>]
</pre><script>Sfdump("{$dumpId}")</script>
HTML,
                $data['data']['payload']['payloads'][0]['content']['values'][0],
            );
    ...
    public function testSendObject(): void
    {
        ...
        $this->broadcastig->assertPushed((string) new EventsChannel('default'), function (array $data) use ($id) {
            ...
            $this->assertSame(
                <<<HTML
<pre class=sf-dump id={$dumpId} data-indent-pad="  ">{<a class=sf-dump-ref>#{$id}</a><samp data-depth=1 class=sf-dump-expanded>
  +"<span class=sf-dump-public title="Runtime added dynamic property">foo</span>": "<span class=sf-dump-str title="3 characters">bar</span>"
</samp>}
</pre><script>Sfdump("{$dumpId}")</script>
HTML,
                $data['data']['payload']['payloads'][0]['content']['values'][0],
